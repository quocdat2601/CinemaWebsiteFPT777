...
namespace MovieTheater.Controllers
{
    ...
    public class BookingController : Controller
    {
        ...
        public async Task<IActionResult> ConfirmTicketForAdmin(string movieId, DateTime showDate, string showTime, List<int>? selectedSeatIds)
        {
            ...
            var allPromotions = _promotionService.GetAll().ToList();
            var filteredPromotions = new List<Promotion>();
            int seatCount = seats.Count;
            foreach (var promo in allPromotions)
            {
                bool addPromo = true;
                if (promo.PromotionConditions != null && promo.PromotionConditions.Any())
                {
                    foreach (var cond in promo.PromotionConditions)
                    {
                        if (cond.TargetField != null && cond.TargetField.ToLower() == "seat")
                        {
                            if (int.TryParse(cond.TargetValue, out int targetValue))
                            {
                                switch (cond.Operator)
                                {
                                    case ">=":
                                        if (!(seatCount >= targetValue)) addPromo = false;
                                        break;
                                    case ">":
                                        if (!(seatCount > targetValue)) addPromo = false;
                                        break;
        ...
        public async Task<IActionResult> ConfirmTicketForAdmin([FromBody] ConfirmTicketAdminViewModel model)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(model.SelectedPromotionId))
                {
                    var selectedPromotion = _promotionService.GetById(int.Parse(model.SelectedPromotionId));
                    if (selectedPromotion != null && selectedPromotion.DiscountLevel.HasValue)
                    {
                        discount += model.BookingDetails.TotalPrice * (selectedPromotion.DiscountLevel.Value / 100m);
                    }
                }
