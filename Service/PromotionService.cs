...
namespace MovieTheater.Service
{
    public class PromotionService : IPromotionService
    {
        ...
        public List<Promotion> GetEligiblePromotionsForMember(string memberId, int seatCount = 0, DateTime? showDate = null, string movieId = null, string movieName = null)
        {
            ...
            foreach (var promotionGroup in conditionsByPromotion)
            {
                ...
                foreach (var condition in conditions)
                {
                    ...
                    {
                        // Check other conditions based on Target_Entity and Target_Field
                        conditionMet = CheckCondition(condition, accountId);
                    }
            ...
            var eligiblePromotions = _context.Promotions
        ...
        private bool CheckCondition(PromotionCondition condition, string accountId)
        {
            try
            {
                if (string.IsNullOrEmpty(condition.TargetEntity) || string.IsNullOrEmpty(condition.TargetField))
                    return true;

                switch (condition.TargetEntity.ToLower())
                {
                    case "invoice":
                        return CheckInvoiceCondition(condition, accountId);
                    case "member":
                        return CheckMemberCondition(condition, accountId);
                    case "account":
                        return CheckAccountCondition(condition, accountId);
                    default:
                        return true; // Unknown entity, assume eligible
                }
            }
            catch (Exception)
            {
                return false; // If there's an error, assume not eligible
            }
        }
        ...
        private bool CheckMemberCondition(PromotionCondition condition, string accountId)
        {
