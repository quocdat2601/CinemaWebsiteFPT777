...
namespace MovieTheater.Tests.Service
{
    public class TicketServiceTests
    {
        ...
        public async Task GetUserTicketsAsync_ReturnsTickets()
        {
            ...
            _invoiceRepoMock.Setup(r => r.GetByAccountIdAsync(accountId, null, null)).ReturnsAsync(invoices);
        ...
        public async Task GetUserTicketsAsync_ReturnsEmptyList_WhenNoTickets()
        {
            // Arrange
            var accountId = "acc2";
            _invoiceRepoMock.Setup(r => r.GetByAccountIdAsync(accountId, null, null)).ReturnsAsync(new List<Invoice>());
            var service = CreateService();
            // Act
            var result = await service.GetUserTicketsAsync(accountId);
            // Assert
        ...
        public async Task GetTicketDetailsAsync_WithSeatIds()
        {
            ...
            // Assert
            Assert.NotNull(result);
            Assert.Equal("inv1", result.Booking.InvoiceId);
        }
        ...
        public async Task GetTicketDetailsAsync_WithSeatNames()
        {
            ...
            _invoiceRepoMock.Setup(r => r.GetDetailsAsync("inv1", "acc1")).ReturnsAsync(booking);
            _seatRepoMock.Setup(r => r.GetSeatByName("A1")).Returns(new Seat { SeatId = 1, SeatName = "A1", SeatType = new SeatType { TypeName = "VIP", PricePercent = 100 } });
            _seatRepoMock.Setup(r => r.GetSeatByName("A2")).Returns(new Seat { SeatId = 2, SeatName = "A2", SeatType = new SeatType { TypeName = "Normal", PricePercent = 80 } });
            var service = CreateService();
            // Act
            var result = await service.GetTicketDetailsAsync("inv1", "acc1");
            // Assert
