...
namespace MovieTheater.Tests.Service
{
    public class BookingDomainServiceTests
    {
        ...
        private readonly Mock<IHubContext<MovieTheater.Hubs.SeatHub>> _seatHubContext = new();
        ...
        public BookingDomainServiceTests()
        {
            ...
            _seatHubContext.Setup(x => x.Clients).Returns(mockClients.Object);
        ...
        private static SeatType MakeSeatType(int id, string name, int pricePercent = 100)
            => new SeatType { SeatTypeId = id, TypeName = name, PricePercent = pricePercent, ColorHex = "#FFFFFF" };

        private static Seat MakeSeat(int id, string name, int seatTypeId, SeatType seatType)
            => new Seat { SeatId = id, SeatName = name, SeatTypeId = seatTypeId, SeatType = seatType };

        private static Account MakeAccount(string id, string fullName = "Test User")
            => new Account { AccountId = id, FullName = fullName, Email = "test@test.com" };

        private static ConfirmBookingViewModel MakeConfirmBookingViewModel(int movieShowId = 1, List<SeatDetailViewModel> seats = null)
            => new ConfirmBookingViewModel
            {
                MovieShowId = movieShowId,
                SelectedSeats = seats ?? new List<SeatDetailViewModel> { new() { SeatId = 1, Price = 100m } }
            };

        private static ConfirmTicketAdminViewModel MakeConfirmTicketAdminViewModel(string memberId = "M123", ConfirmBookingViewModel bookingDetails = null)
            => new ConfirmTicketAdminViewModel
            {
                MemberId = memberId,
                BookingDetails = bookingDetails ?? MakeConfirmBookingViewModel()
            };
        ...
        private void SetupBasicMocks()
        {
            ...
            _seatService.Setup(x => x.GetSeatById(It.IsAny<int>())).Returns(new Seat { SeatId = 1, SeatName = "A1" });
        ...
        private async Task<SeatType> AddSeatTypeToContext(int id, string name = "Standard", int pricePercent = 100)
        {
            var seatType = MakeSeatType(id, name, pricePercent);
            _context.SeatTypes.Add(seatType);
            await _context.SaveChangesAsync();
            return seatType;
        }

        private async Task<Seat> AddSeatToContext(int id, string name, int seatTypeId, SeatType seatType)
        {
            var seat = MakeSeat(id, name, seatTypeId, seatType);
            _context.Seats.Add(seat);
            await _context.SaveChangesAsync();
            return seat;
        }

        private async Task<Member> AddMemberToContext(string memberId, string accountId)
        {
            var account = MakeAccount(accountId);
            var member = new Member { MemberId = memberId, AccountId = accountId, Account = account };
        ...
        public async Task BuildConfirmBookingViewModelAsync_HappyPath_PopulatesBasicFields()
        {
            ...
            var seatType = new SeatType { SeatTypeId = 2, TypeName = "VIP", PricePercent = 200, ColorHex = "#FFFFFF" };
            var seatEntity = new Seat { SeatId = 5, SeatName = "A1", SeatTypeId = 2, SeatType = seatType };
            _context.SeatTypes.Add(seatType);
            ...
            _movieService.Setup(x => x.GetMovieShowById(42)).Returns(show);
            _seatService.Setup(x => x.GetSeatById(5)).Returns(new Seat { SeatId = 5, SeatTypeId = 2, SeatType = seatType });
            ...
            Assert.Equal(200m, vm.SelectedSeats[0].Price);
        ...
        {
            // Arrange
            // Create a valid model but with invalid data to test the validation logic
            ...
            // Mock the service to handle the request
            _bookingService.Setup(x => x.GenerateInvoiceIdAsync()).ReturnsAsync("INV1");
            ...
            Assert.Contains("Invalid booking data", result.ErrorMessage);
        }
        ...
        public async Task ConfirmBookingAsync_HappyPath_CreatesInvoiceAndReturnsSuccess()
        {
            ...
            _bookingService.Setup(x => x.GenerateInvoiceIdAsync()).ReturnsAsync("INV1");
            _movieService.Setup(x => x.GetMovieShowById(1)).Returns(new MovieShow { MovieShowId = 1 });
        ...
        public async Task BuildSuccessViewModelAsync_HappyPath_PopulatesViewModel()
        {
            ...
            // Add the seat and schedule seat with proper linking
            var seatType = new SeatType { SeatTypeId = 1, TypeName = "Standard", PricePercent = 100, ColorHex = "#FFFFFF" };
            _context.SeatTypes.Add(seatType);
            var seat = new Seat { SeatId = 7, SeatName = "A7", SeatTypeId = 1, SeatType = seatType };
            _context.Seats.Add(seat);
            var scheduleSeat = new ScheduleSeat { InvoiceId = "I1", SeatId = 7, MovieShowId = 5 };
            ...
            Assert.Equal(expectedDate, vm.BookingDetails.ShowDate);
            Assert.Contains("A7", vm.BookingDetails.SelectedSeats.Select(s => s.SeatName));
            Assert.Equal(100m, vm.Subtotal + vm.TotalFoodPrice);
        }
        ...
        public async Task BuildConfirmBookingViewModelAsync_FoodLogic_WorksWithValidFoodIdsAndQtys()
        {
            ...
            var show = MakeShow(1, DateOnly.FromDateTime(DateTime.Today), new TimeOnly(14, 0));
            var seatEntity = new Seat { SeatId = 5, SeatName = "A1", SeatTypeId = 2, SeatType = new SeatType { SeatTypeId = 2, TypeName = "VIP", PricePercent = 200, ColorHex = "#FFFFFF" } };
        ...
        public async Task BuildConfirmTicketAdminViewModelAsync_FoodLogic_WorksWithValidFoodIdsAndQtys()
        {
            ...
            _context.MovieShows.Add(show);
            await _context.SaveChangesAsync();
            var food = new Food { FoodId = 1, Name = "Popcorn", Price = 10, Category = "Snacks" };
            ...
            // Act
            var vm = await _svc.BuildConfirmTicketAdminViewModelAsync(1, new List<int> { 1 }, new List<int> { 1 }, new List<int> { 2 });

            // Assert
            Assert.NotNull(vm);
            Assert.Single(vm.SelectedFoods);
            Assert.Equal(1, vm.SelectedFoods[0].FoodId);
            Assert.Equal(2, vm.SelectedFoods[0].Quantity);
        }
        ...
        {
            // Arrange
            var model = MakeConfirmTicketAdminViewModel("M123");
        ...
        public async Task ConfirmTicketForAdminAsync_ReturnsFalse_WhenVoucherIsInvalid()
        {
            ...
            await _context.SaveChangesAsync();
            var model = MakeConfirmTicketAdminViewModel("M123");
            model.SelectedVoucherId = "VOUCHER1";
            _voucherService.Setup(x => x.GetById("VOUCHER1")).Returns((Voucher)null);
            SetupBasicMocks();
            await AddSeatTypeToContext(1);
            await AddSeatToContext(1, "A1", 1, _context.SeatTypes.First());
        ...
        {
            // Arrange
            await AddMemberToContext("M123", "A1");
            var model = MakeConfirmTicketAdminViewModel("M123");
            model.CustomerType = "member";
            model.UsedScore = 0;
            model.VoucherAmount = 0;
        ...
        public async Task ConfirmTicketForAdminAsync_UpdatesVoucher_WhenNotUsed()
        {
            // Arrange
            await AddMemberToContext("M123", "A1");
            var voucher = new Voucher { VoucherId = "VOUCHER1", IsUsed = false, AccountId = "A1", Code = "VOUCHER1" };
        ...
        {
            // Arrange
            var account = new Account { AccountId = "A1", Email = "test@test.com" };
            var member = new Member { MemberId = "M123", AccountId = "A1", Account = account };
            var voucher = new Voucher { VoucherId = "VOUCHER1", IsUsed = true, AccountId = "A1", Code = "VOUCHER1" };
            _context.Accounts.Add(account);
            _context.Members.Add(member);
            _context.Vouchers.Add(voucher);
            await _context.SaveChangesAsync();
            var model = new ConfirmTicketAdminViewModel
            {
                ...
                { 
                    SelectedSeats = new List<SeatDetailViewModel> { new() { SeatId = 1 } }, 
            ...
            // Assert
            Assert.True(result.Success);
            Assert.True(voucher.IsUsed); // Should remain true
        ...
        public async Task ConfirmTicketForAdminAsync_SavesFoodInvoices_WhenFoodsPresent()
        {
            ...
            var model = MakeConfirmTicketAdminViewModel("M123");
            model.CustomerType = "member";
            model.SelectedFoods = new List<FoodViewModel> { new() { FoodId = 1, Quantity = 2, Price = 50 } };
            ...
            // Assert
            Assert.True(result.Success);
            var foodInvoice = _context.FoodInvoices.FirstOrDefault(f => f.FoodId == 1);
        ...
        public async Task BuildTicketBookingConfirmedViewModelAsync_HandlesRankDiscountAndVoucher()
        {
            ...
            var rank = new Rank { RankId = 1, DiscountPercentage = 10, ColorGradient = "#000000-#FFFFFF", IconClass = "icon-star" };
